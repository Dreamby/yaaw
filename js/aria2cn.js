  ...
  return {
    init: function(path, onready) {
      ...
      if (jsonrpc_interface.indexOf("http") == 0) {
        ...
        jsonrpc_ws.onerror = function(event) {
          ...
          main_alert("alert-error", "websocket 错误。你可能需要刷新这个页面以重启。");
          ws_callback = {};
        };
    ...
    pause_all: function() {
      ARIA2.request("pauseAll", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "已暂停所有任务。请等待操作，譬如联系 BitTorrent tracker。", 2000);
        }
      );
    },
    ...
    purge_download_result: function() {
      ARIA2.request("purgeDownloadResult", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "移除所有已完成/出错/已移除的下载任务。", 2000);
        }
      );
    },
    ...
    auto_refresh: function(interval) {
      ...
      interval_id = window.setInterval(function() {
        ...
        if (select_lock) {
          if (need_refresh) {
            main_alert("", "从上次更改以来任务列表已发生变化。点击【刷新】按钮更新任务列表。");
          }
        } else {
          if (need_refresh) {
            ARIA2.refresh();
